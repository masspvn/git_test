...
# Sau đây là cách mở file Hanoi.csv bằng Python. Ta sẽ lưu dữ liệu của file này vào list
# monthly_weather
monthly_weather = []
...
for element in monthly_weather:
   print(element, type(element))

#It can be read from the output that "monthly_weather" list contains elements whose type are <class 'collections.OrderedDict'>
...
bestsofar = 0
for row in monthly_weather:
   if int(row["rainy_days"]) >= bestsofar:
      bestsofar = int (row["rainy_days"])
      best_month = row["month"]

print(best_month)

#the output says that the month which has the greatest number of rainy days is "8" (August)